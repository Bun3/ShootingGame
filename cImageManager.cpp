...
void cImageManager::Render(cTexture* texture, Vec2 pos, Vec3 size, float rot, bool center, D3DCOLOR color)
{
	D3DXMATRIX mat;
	...
	m_sprite->SetTransform(&mat);
	if (!center) {
		m_sprite->Draw(texture->texturePtr, NULL, NULL, NULL, color);
	}
	else {
